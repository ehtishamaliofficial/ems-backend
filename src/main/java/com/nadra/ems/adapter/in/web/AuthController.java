package com.nadra.ems.adapter.in.web;
 ...
 * <p>
 * Login always returns a scoped token. Full access tokens are only issued
 * after completing 2FA verification or setup.
 * <p>
 * All public endpoints are under {@code /api/v1/auth/} and configured
 * in {@code SecurityConfig}.
 */
@RestController
@RequestMapping("/api/v1/auth")
@Tag(name = "Authentication", description = "Endpoints for user registration, login, mandatory 2FA, and token management")
public class AuthController {
    ...
    @Operation(summary = "Login to the system",
            description = "Authenticates a user by username or email. Always returns a scoped token. " +
                    "If twoFactorEnabled=true, use the token with /verify-2fa. " +
                    "If twoFactorEnabled=false, use the token with /2fa/setup then /2fa/enable.")
    @PostMapping("/login")
    ...
    @Operation(summary = "Verify 2FA token",
            description = "Verifies the TOTP code for users who have 2FA enabled. " +
                    "Requires a scoped Bearer token from the login response. " +
    ...
    public ResponseEntity<ApiResponse<TokenResponse>> verifyTwoFactor(
        ...
        String scopedToken = authHeader.replace("Bearer ", "");
        String clientIp = getClientIp(httpRequest);
    ...
    @Operation(summary = "Setup 2FA",
            description = "Generates a 2FA secret and QR code for the user. " +
                    "Requires a scoped Bearer token (2FA_SETUP) from the login response.",
            security = @SecurityRequirement(name = "Bearer Authentication"))
    ...
    @Operation(summary = "Enable 2FA",
            description = "Enables 2FA after validating the TOTP code. " +
                    "Requires a scoped Bearer token (2FA_SETUP). " +
                    "Returns full access + refresh tokens on success.",
            security = @SecurityRequirement(name = "Bearer Authentication"))
